...
class ResearchArchitectureRecord(Base):
    ...
    v3_phase3_dataset_id: Mapped[str | None] = mapped_column(String(100))
...
class V3Phase3ExperimentRecord(Base):
    __tablename__ = "v3_phase3_experiment_locks"
    ...
    control_configuration: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)
    candidate_configuration: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)
    control_architecture_hash: Mapped[str | None] = mapped_column(String(64))
    candidate_architecture_hash: Mapped[str | None] = mapped_column(String(64))
    promotion_policy_hash: Mapped[str | None] = mapped_column(String(64))
    semantic_index_identity: Mapped[str] = mapped_column(String(64), nullable=False)
    corpus_identity: Mapped[str] = mapped_column(String(64), nullable=False)
    embedding_preflight: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    hosted_preflight: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    shared_traces: Mapped[list[dict[str, Any]] | None] = mapped_column(JSON)
    control_rows: Mapped[list[dict[str, Any]] | None] = mapped_column(JSON)
    candidate_rows: Mapped[list[dict[str, Any]] | None] = mapped_column(JSON)
    control_metrics: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    candidate_metrics: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    paired_deltas: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    recovery_funnel: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    instruction_boundary: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    prompt_injection: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    category_results: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    retrieval_metrics: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    failure_census: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    security: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    citations: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    reliability: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    latency: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    usage: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    cost: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    selection: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    selected_strategy: Mapped[str | None] = mapped_column(String(120))
...
class V3Phase5bExperimentRecord(Base):
    __tablename__ = "v3_phase5b_experiment_locks"
    ...
    v3_status: Mapped[str | None] = mapped_column(String(40))
    primary_remaining_bottleneck: Mapped[str | None] = mapped_column(String(80))
    reference_configuration: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    reference_rows: Mapped[list[dict[str, Any]] | None] = mapped_column(JSON)
    reference_metrics: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    candidate_b_rows: Mapped[list[dict[str, Any]] | None] = mapped_column(JSON)
    candidate_b_metrics: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    ranking_metrics: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    ranking_comparison: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    candidate_ranking_hash: Mapped[str | None] = mapped_column(String(64))
    dataset_frozen_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    selection_policy_frozen_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
    )
