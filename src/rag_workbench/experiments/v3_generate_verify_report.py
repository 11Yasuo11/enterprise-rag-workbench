...
def render_v3_phase1_markdown(status: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"Unsupported recovered answers {diagnostic.get('unsupported_recovered_answers', (diagnostic.get('rollup') or {}).get('unsupported_recovery_count', 0))}. Version violations {diagnostic.get('version_violations', 0)}.",
            f"GO / NO_GO: `{NO_GO if (go or '') in {'NO_GO_FOR_UNSEEN_EXPERIMENT', 'NO_GO_FOR_UNSEEN_V3_PHASE1'} else go}`.",
            "",
            "Diagnostic category rescues (historical FN traces, not promotion evidence):",
