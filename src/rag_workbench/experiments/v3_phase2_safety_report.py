...
def render_v3_phase2_markdown(status: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            f"Experiment 1 status `{status.get('experiment_1_status') or 'EXP1_HOSTED_INCOMPLETE'}`.",
            f"Stop reason `{status.get('stop_reason')}`.",
            f"V3 index identity `{(preflight or {}).get('v3_index_identity')}`.",
            f"Existing query-embedding cache hits `{(preflight or {}).get('current_v3_embedding_cache_hits_for_validation_queries')}`.",
            f"New document embedding HTTP calls `{(preflight or {}).get('new_document_embedding_http_calls')}`.",
            f"New query embeddings `{(preflight or {}).get('new_query_embeddings')}`.",
            f"New Judge/Draft/Verifier worst case `{(preflight or {}).get('new_judge_calls_worst_case')}` / `{(preflight or {}).get('new_draft_calls_worst_case')}` / `{(preflight or {}).get('new_verifier_calls_worst_case')}`.",
            f"Maximum physical attempts `{(preflight or {}).get('maximum_physical_attempts')}`.",
            f"Official Sol worst-case USD `{_fmt((preflight or {}).get('estimated_usd_official_sol_worst_case'))}`.",
            f"Official Sol floor USD `{_fmt((preflight or {}).get('estimated_usd_official_sol_floor'))}`.",
            f"Embedding USD (separate) `{_fmt((preflight or {}).get('estimated_usd_embedding_separate'))}`.",
            f"Cumulative Judge ceiling `{(preflight or {}).get('cumulative_judge_ceiling_required')}`.",
            f"Cumulative embedding ceiling `{(preflight or {}).get('cumulative_embedding_ceiling_required')}`.",
